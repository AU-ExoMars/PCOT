...
% Parallel ``code'' and comment stuff, inside a paracol{2} env.

\newcommand{\parblock}[2]{
{\small \texttt{#1}}
\switchcolumn
\noindent #2\vspace*{0.6em}
\switchcolumn*
}
...
\input{install.tex}
\input{model.tex}
\input{customisation.tex}
\input{writingfuncs.tex}
\input{writingnodes.tex}

\section{Todo}
\begin{itemize}
\item How the canvas works in more detail, including the canvas persistence mechanism
\item How the data is actually stored --- what the numbers in the uncertainty and DQ arrays
really mean and how they are imported, particularly from PDS4
\item Uncertainty calculations (also need to be added to example in Sec.~\ref{writingnodes})
\item More details on hooks and the new hook decorators
\end{itemize}
