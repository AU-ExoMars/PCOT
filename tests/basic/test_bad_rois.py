...
def test_filter_bad_roi_all_pixels_bad():
    """Test an ROI with all pixels bad is indeed filtered out."""
    # Create an imagecube with 4 bands
    ic = ImageCube(np.zeros((100, 100, 4), dtype=np.float32))

    # create a circular ROI and use it to set bad pixels in the image
    roi_to_set = ROICircle(30,30, 20)
    ss = ic.subimage(roi=roi_to_set)
    dqs = ss.maskedDQ(maskBadPixels=True)
    dqs |= dq.SAT    # we are editing the subimage - relies on the returned value holding slices, not copy.

    # add a circular ROI
    ic.rois.append(ROICircle(40, 40, 20, label="good-circ"))
    # and a rectangular one
    ic.rois.append(ROIRect(rect=(60, 60, 20, 20), label="good-rect"))
    # add another circle; the pixels are all bad
    ic.rois.append(ROICircle(30, 30, 20, label="bad-circ"))
    ...
    assert "good-circ" in labels
    assert "good-rect" in labels
    assert "bad-circ" not in labels


def test_filter_bad_roi_all_pixels_in_one_bandbad():
    """Test an ROI with one band having all pixels bad is indeed filtered out."""
    # Create an imagecube with 4 bands
    ic = ImageCube(np.zeros((100, 100, 4), dtype=np.float32))

    # this time we create a rect ROI and use it to set bad pixels in the image
    roi_to_set = ROIRect(rect=(30, 30, 20, 20))
    ss = ic.subimage(roi=roi_to_set)
    dqs = ss.maskedDQ(maskBadPixels=True)
    dqs |= dq.SAT    # we are editing the subimage - relies on the returned value holding slices, not copy.
    ...
    # add a circular ROI
    ic.rois.append(ROICircle(40, 40, 20, label="good-circ"))
    # add the should-be-bad ROI
    ic.rois.append(ROIRect(rect=(30, 30, 20, 20), label="bad-rect"))
    # and a rectangular one
    ic.rois.append(ROIRect(rect=(60, 60, 20, 20), label="good-rect"))
