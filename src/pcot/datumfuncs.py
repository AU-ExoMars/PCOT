...
@datumfunc
def decorrstretch(img, stretch=2.5, clip_percent=5):
    """
    ...
    **Ignores DQ and uncertainty**
    ...
    @param img:img:input image - algorithm is only applied to ROIs if ROIs present
    @param stretch:number:stretch applied to principal components  (default 1)
    @param clip_percent:number:percentile of outliers to clip in final normalisation (default 5)
    """
    img = img.get(Datum.IMG)
    if img is None:
        return None
    ...
    import pcot.utils.decorr
    subimage = img.subimage()
    newimg, _ ,_ = pcot.utils.decorr.decorrelation_stretch(subimage.img, subimage.mask, stretch, clip_percent)
