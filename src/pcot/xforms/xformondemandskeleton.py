...
"""
import numpy as np
from PySide2.QtWidgets import QWidget, QGridLayout, QLabel, QSlider, QDoubleSpinBox, QComboBox, QSizePolicy, QPushButton
...
@xformtype
class XFormOnDemandSkeleton(XFormType):
    """
    This object is not a node, but the singleton that nodes of this type point to
    to control their behaviour. This is identical (and copied from) the XFormSkeleton, but
    rather than running whenever the inputs change (i.e. whenever upstream nodes run) it will
    only run when the user clicks the "run" button in the UI. This is useful for nodes that
    ...
    def __init__(self):
        ...
        super().__init__("skeletonondemand", "testing", "0.0.0")
        self.hasEnable = True   # TODO
    ...
    def perform(self, node):
        """
        This is the same as in the XFormSkeleton, but it doesn't run when the inputs change because enabled is false.
        """
    ...
    def createTab(self, node, window):
        ...
        """
        return TabSkeletonOnDemand(node, window)


class TabSkeletonOnDemand(pcot.ui.tabs.Tab):
    ...
    def __init__(self, node, window):
        ...
        # add the button that will cause the node to run
        self.runButton = QPushButton("Run", self.w)
        self.runButton.setStyleSheet("background-color:rgb(255,100,100)")
        layout.addWidget(self.runButton, 2, 0, 2, 2)
        ...
        self.runButton.clicked.connect(self.onRunPressed)
    ...
    def onRunPressed(self):
        """
        This will cause the node to run. Yes, it's ugly.
        """
        # TODO this is a hack, probably put this entire method as a method in XForm or an optional argument
        # TODO also need to redraw graph ?
        self.nodeChanged()
