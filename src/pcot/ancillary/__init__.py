...
Ancillary data is typically per-band, although we may also need to support data for the cube as a whole.

A typical example of ancillary data is the exposure time of a particular band.

The data is read in this package, but reading is invoked by the image loaders in pcot.dataformats.load
(e.g. load.multifile(), which the Multifile input method uses). Reading needs to be very
flexible - both in determining the data source (e.g. the name of the parallel .xml file to a .png file)
