...
class FilterOnlySource(Source):
    ...
    def __init__(self, filtOrName=None):
        """Constructor takes an optional filter"""
    ...
    def long(self) -> Optional[str]:
        ...
        if isinstance(self.filterOrName, Filter):
            return f"wavelength {int(self.filterOrName.cwl)}, fwhm {int(self.filterOrName.fwhm)}"
        else:
            return f"band {self.filterOrName}"

    def brief(self, captionType=DocumentSettings.CAP_DEFAULT) -> Optional[str]:
        if isinstance(self.filterOrName, Filter):
            return getFiltStr(self.filterOrName, captionType)
        else:
            return f"{self.filterOrName}"
