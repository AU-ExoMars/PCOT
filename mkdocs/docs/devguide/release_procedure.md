...
2. Create a new version number and codeword. For the version number, use   [semantic versioning](https://semver.org). This means:
    ...
    * The version codeword is a helpful mnemonic. We use [Megalithic sites in the UK](https://m.megalithic.co.uk/asb_mapsquare.php) running through letters of the alphabet and roughly south to north, trying to stick with more memorable names. 
...
You may also need to change the second line in this file, which specifies the oldest document versions we can load (i.e. the version of  PCOT which created that document).
